"""Sketch Curves PFM — A premium drawing algorithm for DrawingBotV3.

Algorithm:
  Similar to Sketch Lines, but instead of straight polylines, 
  fits a smooth Catmull-Rom spline to the discovered path.
"""
...
from app.core.geometry import Path, Point, catmull_rom_chain


class SketchCurvesPFM(PathFindingModule):
    """Transforms an image into smooth curves using brightness data."""
    ...
    def description(self) -> str:
        return "Transforms an image into smooth curves using brightness data."
    ...
    def _define_settings(self) -> List[PFMSetting]:
        ...
        settings.extend([
            PFMSetting("curve_smoothness", "Curve Smoothness", SettingType.INTEGER, 10, 1, 50,
                       category="Curves", tooltip="Number of segments to generate per span."),
            PFMSetting("curve_alpha", "Curve Alpha", SettingType.NUMBER, 0.5, 0.0, 1.0, step=0.1,
                       category="Curves", tooltip="0=Uniform, 0.5=Centripetal, 1=Chordal."),
        ])
    ...
    def _process(self, image: np.ndarray,
        ...
        curve_smoothness = max(1, int(self.get("curve_smoothness") or 10))
        curve_alpha = self.get("curve_alpha") or 0.5
        ...
        while iteration < max_iterations:
            ...
            if len(squiggle) >= 2:
                # Convert the squiggle points to a smooth Catmull-Rom curve
                smooth_curve = catmull_rom_chain(squiggle, curve_smoothness, curve_alpha)
                scaled = [(px * sx, py * sy) for px, py in smooth_curve]
                geometries.append(DrawingGeometry(path=scaled, pen_index=0))

            iteration += 1
            if iteration % 50 == 0:
                progress(density_progress, len(geometries),
                         f"Drawing Curves... {len(geometries)} shapes, "
                         f"{density_progress*100:.0f}% density")

        return geometries

    def _erase_line(self, img: np.ndarray, x0: float, y0: float, x1: float, y1: float,
