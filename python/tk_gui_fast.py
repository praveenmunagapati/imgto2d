"""
Minimal Tkinter GUI (fast preview) for imgto2d — runs PFMs at low-res for instant previews.
Run: .venv\\Scripts\\python.exe tk_gui_fast.py
"""

import threading
import sys
from pathlib import Path
from tkinter import Tk, Frame, Button, Label, Canvas, filedialog, ttk, StringVar, BooleanVar, Checkbutton
...
class TkApp:
    def __init__(self, root):
        self.root = root
        root.title('imgto2d - Tkinter Fast Preview')
        ...
        self.fast_preview_var = BooleanVar(value=True)
        self.fast_preview_cb = Checkbutton(ctrl, text='Fast Preview (downsample)', variable=self.fast_preview_var)
        self.fast_preview_cb.pack(anchor='w', pady=4)

        Label(ctrl, text='Preview resolution:').pack(anchor='w', pady=(8,0))
        self.preview_resolution_var = StringVar(value='1080p')
        self.preview_resolution_combo = ttk.Combobox(
            ctrl,
            textvariable=self.preview_resolution_var,
            state='readonly',
            values=['720p', '1080p', '1440p', '2160p', 'Native']
        )
        self.preview_resolution_combo.current(1)
        self.preview_resolution_combo.pack(fill='x', pady=4)
    ...
    def _show_image(self, img_np):
        # Convert grayscale numpy to PIL Image and fit to canvas size while preserving aspect ratio
        h, w = img_np.shape[:2]
    ...
    def start_processing(self):
        ...
        # Disable start button until processing completes
    ...
    def _run_pfm(self, pfm_cls):
        try:
            pfm = pfm_cls()
            # Optionally run on a downsampled image for fast preview
            img_for_pfm = self.processed_image
            if self.fast_preview_var.get() and img_for_pfm is not None:
                h, w = img_for_pfm.shape[:2]
                target = 1920
                scale = max(1, max(h, w) / target)
                if scale > 1:
                    nw = max(1, int(w / scale))
                    nh = max(1, int(h / scale))
                    img_for_pfm = cv2.resize(img_for_pfm, (nw, nh), interpolation=cv2.INTER_AREA)

            geoms = pfm.process(img_for_pfm)
            # Render geometries to image
            if img_for_pfm is None:
                h, w = (800, 600)
            else:
                h, w = img_for_pfm.shape[:2]
